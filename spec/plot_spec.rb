...
describe SVG::Graph::Plot do
  ...
  describe '#burn' do
    ...
    context 'graph options' do
      ...
      context 'popups' do
        let(:series_count) { 1 }
        let(:data) do # TODO: we only need to do this because data is *altered* when the SVG is generated. That needs to be fixed.
          data = series.first[:data]
          data.first.kind_of?(Array) ? data.first.zip(data.last).flatten : data
        end
        let(:options) do
          super().merge(
            add_popups: true,
        ...
        context ':round_popups' do
          context 'false' do
            ...
            context 'text descriptions provided' do
              ...
              let(:series) { super().tap {|series| series.first.merge! description: descriptions.dup } } # TODO: apparently the :description argument gets altered! This should be fixed...

              # TODO: we need this only because data is sorted by X coordinate, and we need some predictability as to which description goes with which popup. This is probably a bug; I think the descriptions should go with the data in the order declared, not the order of the X coordinates.
              let(:sort_data) { true }

              it 'shows text descriptions if provided' do |example|
